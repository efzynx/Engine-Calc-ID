    ...
    try {
      ...
      if (additionalUserInfo?.isNewUser) {
        await setDoc(doc(db, "users", user.uid), {
          ...
          photoURL: user.photoURL, status: "Free User", calculationCount: 0,
    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-100">
      <div className="w-full max-w-md rounded-xl bg-white p-8 shadow-lg">
        ...
        <form onSubmit={handleLogin} className="mt-8 space-y-4">
          ...
          <div><label className="block text-sm font-medium text-slate-700">Password</label><input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required className="mt-1 w-full rounded-md border-slate-300 p-3 shadow-sm text-slate-900"/></div>
          {error && <p className="text-sm text-red-600">{error}</p>}
          <button type="submit" className="w-full flex justify-center items-center gap-2 rounded-lg bg-blue-600 px-4 py-3 text-lg font-bold text-white shadow-md hover:bg-blue-700"><Mail size={20}/> Login dengan Email</button>
        </form>
