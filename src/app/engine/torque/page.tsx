...
import { SlidersHorizontal, Info, ChevronDown, ToyBrick } from 'lucide-react';
...
const KalkulatorTorque: React.FC = () => {
  ...
  const handleCalculate = async () => {
    ...
    const horsepowerNum = parseFloat(horsepower);
    const rpmNum = parseFloat(rpm);

    if (isNaN(horsepowerNum) || isNaN(rpmNum) || horsepowerNum <= 0 || rpmNum <= 0) {
      setError("Mohon masukkan nilai Horsepower (HP) dan RPM yang valid.");
      setTorque(null);
      return;
    }

    // Torque Formula: (Horsepower * 5252) / RPM
    const calculatedTorque = (horsepowerNum * 5252) / rpmNum;
    ...
    <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
      ...
      <div className="lg:col-span-3">
        <div className="w-full rounded-xl bg-white p-6 sm:p-8 shadow-lg">
          <div className="mb-6 p-4 bg-blue-50 border-l-4 border-blue-400 rounded-r-lg"><div className="flex"><div className="flex-shrink-0"><Info className="h-5 w-5 text-blue-400" /></div><div className="ml-3"><p className="text-sm text-blue-700"><b>Torsi (Torque)</b> adalah ukuran gaya putar yang dihasilkan oleh mesin, yang mempengaruhi akselerasi.</p></div></div></div>
          <div className="space-y-4">
            <div><label htmlFor="horsepower" className="block text-sm font-medium text-slate-700 mb-1">Tenaga / Horsepower (HP)</label><input type="number" id="horsepower" value={horsepower} onChange={(e) => setHorsepower(e.target.value)} placeholder="Contoh: 150" className="w-full rounded-md border-slate-300 bg-white p-3 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-slate-900"/></div>
      ...
      <div className="lg:col-span-2 space-y-6">
        <div className="rounded-xl bg-white shadow-lg p-6">
          <h2 className="text-lg font-bold text-slate-800">Estimasi Torsi:</h2>
          <p className="text-5xl font-extrabold text-green-600 mt-2">{torque ? torque.toFixed(2) : '--'} <span className="text-3xl font-medium text-slate-500">ft-lbs</span></p>
        </div>
...
export default function TorquePage() {
  return (
    <AppLayout title="Kalkulator Engine Torque">
      <KalkulatorTorque />
    </AppLayout>
  );
}
