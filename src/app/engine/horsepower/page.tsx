...
import { SlidersHorizontal, Info, ChevronDown, Zap } from 'lucide-react';
...
const KalkulatorHorsepower: React.FC = () => {
  const { user, userProfile } = useAuth();

  const [torque, setTorque] = useState<string>(''); // In ft-lbs
  ...
  const handleCalculate = async () => {
    ...
    if (isNaN(torqueNum) || isNaN(rpmNum) || torqueNum <= 0 || rpmNum <= 0) {
      setError("Mohon masukkan nilai Torsi (ft-lbs) dan RPM yang valid.");
      setHorsepower(null);
      return;
    }

    // Horsepower Formula: (Torque * RPM) / 5252
    const calculatedHp = (torqueNum * rpmNum) / 5252;
    setHorsepower(calculatedHp);
    ...
    <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
      ...
      <div className="lg:col-span-3">
        <div className="w-full rounded-xl bg-white p-6 sm:p-8 shadow-lg">
          <div className="mb-6 p-4 bg-blue-50 border-l-4 border-blue-400 rounded-r-lg"><div className="flex"><div className="flex-shrink-0"><Info className="h-5 w-5 text-blue-400" /></div><div className="ml-3"><p className="text-sm text-blue-700"><b>Horsepower (HP)</b> adalah satuan pengukuran daya yang menunjukkan tingkat kerja yang dapat dilakukan oleh mesin.</p></div></div></div>
          <div className="space-y-4">
            <div><label htmlFor="torque" className="block text-sm font-medium text-slate-700 mb-1">Torsi / Torque (ft-lbs)</label><input type="number" id="torque" value={torque} onChange={(e) => setTorque(e.target.value)} placeholder="Contoh: 120" className="w-full rounded-md border-slate-300 bg-white p-3 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-slate-900"/></div>
            <div><label htmlFor="rpm" className="block text-sm font-medium text-slate-700 mb-1">Putaran Mesin (RPM)</label><input type="number" id="rpm" value={rpm} onChange={(e) => setRpm(e.target.value)} placeholder="Contoh: 5500" className="w-full rounded-md border-slate-300 bg-white p-3 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-slate-900"/></div>
          </div>
      ...
      <div className="lg:col-span-2 space-y-6">
        <div className="rounded-xl bg-white shadow-lg p-6">
          <h2 className="text-lg font-bold text-slate-800">Estimasi Tenaga:</h2>
          <p className="text-5xl font-extrabold text-green-600 mt-2">{horsepower ? horsepower.toFixed(2) : '--'} <span className="text-3xl font-medium text-slate-500">HP</span></p>
        </div>
...
export default function HorsepowerPage() {
  return (
    <AppLayout title="Kalkulator Engine Horsepower">
      <KalkulatorHorsepower />
    </AppLayout>
  );
}
